...
onMounted(() => {
    isResetError.value = !!error.value;
    tryingToReset.value = !!status.value;
});

const forget = async () => {
    processing = true;
    await axios
        .post("/forget-password", { email: email.value })
        .then(({ data }) => {
            if (data.success == true && data.message == "success") {
                status.value = "Password reset link send in your email.";
